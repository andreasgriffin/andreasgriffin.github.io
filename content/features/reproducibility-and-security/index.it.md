...
- **Custodia incentrata sull'hardware**: Bitcoin Safe richiede [firmatari hardware]({{< ref "knowledge/supported-hardware-signers" >}}), cosi i seed restano su dispositivi dedicati invece di diventare segreti esposti su un computer generico.
- **Flusso di configurazione piu sicuro**: La procedura guidata di configurazione aiuta a creare wallet davvero pronti all'uso reale, includendo test dei [firmatari hardware]({{< ref "knowledge/supported-hardware-signers" >}}), [schede PDF di backup]({{< ref "features/pdf-export/" >}}) e indicazioni su come conservare firmatari hardware e backup dei seed.
- **Verifica degli indirizzi di ricezione**: Bitcoin Safe rende semplice [verificare gli indirizzi di ricezione]({{< ref "knowledge/verify-receive-address/" >}}) sul firmatario stesso prima di condividerli.
- **Rilevamento dell'address poisoning**: Avvisa in presenza di [indirizzi sospetti simili]({{< ref "features/address-poisoning/" >}}).
...
- **Collaborazione che rende il multisig piu semplice**: Bitcoin Safe rende pratica la [collaborazione multisignature]({{< ref "features/collaboration/" >}}), cosi coordinare configurazione e firma diventa molto piu facile nell'uso reale.
