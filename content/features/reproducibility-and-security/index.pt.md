...
- **Custodia centrada em hardware**: O Bitcoin Safe exige [assinantes de hardware]({{< ref "knowledge/supported-hardware-signers" >}}), para que as seeds permaneçam em dispositivos dedicados em vez de se tornarem segredos expostos num computador de uso geral.
- **Fluxo de configuracao mais seguro**: O assistente de configuracao ajuda a criar wallets realmente prontas para uso real, incluindo testes de [assinantes de hardware]({{< ref "knowledge/supported-hardware-signers" >}}), [folhas PDF de backup]({{< ref "features/pdf-export/" >}}) e orientacoes sobre como guardar os assinantes de hardware e os backups de seed.
- **Verificacao de enderecos de rececao**: O Bitcoin Safe facilita [verificar enderecos de rececao]({{< ref "knowledge/verify-receive-address/" >}}) no proprio assinante antes de os partilhar.
- **Detecao de envenenamento de enderecos**: Avisa sobre [enderecos suspeitos e parecidos]({{< ref "features/address-poisoning/" >}}).
...
- **Colaboracao que torna a multisig mais facil**: O Bitcoin Safe torna a [colaboracao multisig]({{< ref "features/collaboration/" >}}) pratica, tornando muito mais facil coordenar a configuracao e a assinatura no mundo real.
