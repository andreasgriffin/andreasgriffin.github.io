...
- **Hardware-zentrierte Verwahrung**: Bitcoin Safe setzt auf [Hardware-Signer]({{< ref "knowledge/supported-hardware-signers" >}}), damit Seeds auf dedizierten Geräten bleiben und nicht zu Hot Secrets auf einem allgemeinen Computer werden.
- **Sichererer Einrichtungsablauf**: Der Einrichtungsassistent hilft dabei, Wallets zu erstellen, die wirklich für den realen Einsatz bereit sind, einschließlich Tests für [Hardware-Signer]({{< ref "knowledge/supported-hardware-signers" >}}), [PDF-Backup-Blättern]({{< ref "features/pdf-export/" >}}) und Hinweisen zur sicheren Aufbewahrung von Hardware-Signern und Seed-Backups.
- **Verifikation von Empfangsadressen**: Bitcoin Safe macht es einfach, [Empfangsadressen]({{< ref "knowledge/verify-receive-address/" >}}) direkt auf dem Signer zu prüfen, bevor sie geteilt werden.
- **Erkennung von Address-Poisoning**: Die App warnt vor verdächtigen [ähnlich aussehenden Adressen]({{< ref "features/address-poisoning/" >}}).
...
- **Zusammenarbeit, die Multisig einfacher macht**: Bitcoin Safe macht [Multisignatur-Zusammenarbeit]({{< ref "features/collaboration/" >}}) praxistauglich, sodass Einrichtung und Signieren in realen Abläufen deutlich einfacher zu koordinieren sind.
