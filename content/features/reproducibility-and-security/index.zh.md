...
- **以硬件为核心的托管方式**：Bitcoin Safe 要求使用[硬件签名设备]({{< ref "knowledge/supported-hardware-signers" >}})，这样 seed 就能保留在专用设备上，而不会变成通用电脑中的热秘密。
- **更安全的设置流程**：设置向导帮助用户创建真正适合实际使用的钱包，包括[硬件签名设备]({{< ref "knowledge/supported-hardware-signers" >}})测试、[PDF 备份页]({{< ref "features/pdf-export/" >}})，以及关于如何保存设备和 seed 备份的指导。
- **接收地址验证**：Bitcoin Safe 让你在分享之前，能够直接在签名设备上轻松[验证接收地址]({{< ref "knowledge/verify-receive-address/" >}})。
- **地址投毒检测**：它会对可疑的[相似地址]({{< ref "features/address-poisoning/" >}})发出警告。
- **multisig 安全性**：Bitcoin Safe 支持 multisig 钱包，因此更大的资金可以由多个设备或多人共同保护，而不是依赖单一故障点。
- **让 multisig 更容易使用的协作能力**：Bitcoin Safe 让[multisig 协作]({{< ref "features/collaboration/" >}})变得切实可用，因此在实际使用中协调设置和签名会容易得多。
